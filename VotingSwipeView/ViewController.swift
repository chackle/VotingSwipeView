...
class ViewController: UIViewController {
  ...
  private let lowestWidthRatio = CGFloat(0.5)
  ...
  private var pageWidth: CGFloat {
    return self.panGestureView.bounds.width
  }
  private var currentSettledSide = Direction.neutral
  
  func setup() {
    let gestureRecognizer = UIPanGestureRecognizer(target: self, action: #selector(ViewController.handlePan(sender:)))
